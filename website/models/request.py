...
class Request(db.Model):
    ...
    cer_date = db.Column(db.Date, nullable=False)
    ...
    completed_by = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    completed_by_user = db.relationship('User', foreign_keys=[completed_by], backref='completed_requests')

    user = db.relationship('User', foreign_keys=[user_id], back_populates='request', lazy=True)
