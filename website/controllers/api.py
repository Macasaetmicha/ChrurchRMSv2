...
def register_begin():
    ...
    if flask_login.current_user.fido_info != "":
        print(f"User already has a fido token registered. {flask_login.current_user.fido_info}")
        return jsonify({"status": "error", "message": "fido has already been activated"}), 400
    else:
        print(f"User does not have a fido token registered. {flask_login.current_user.fido_info}")
    print('Current User Options:')
    print(dir(flask_login.current_user))
    ...
    options, state = fido_server.register_begin(
        PublicKeyCredentialUserEntity(
            ...
            display_name=f'{flask_login.current_user.first_name} {flask_login.current_user.last_name}',
        ),
    ...
    active_challenges[flask_login.current_user.id] = state
...
def register_complete():
    ...
    # valid information.
    fido_state = active_challenges.pop(flask_login.current_user.id)
    if fido_state is None:
        print(f"No fido_state found for user_id: {flask_login.current_user.id}")
    ...
    try:
        set_fido_info(user_id, auth_data.hex())
        print(f"FIDO info set successfully for user_id: {user_id}")
    except Exception as e:
        print(f"Error saving FIDO info for user_id {user_id}: {e}")
        return jsonify({"status": "error", "message": "failed to save FIDO info"}), 500
